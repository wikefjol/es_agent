...
class TestOrchestratorAgent:
    ...
    def test_orchestrator_initializes_with_custom_components(
        self, mock_planner, mock_executor, mock_tool_registry
    ):
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_identifies_new_query_requiring_tools(
        self, orchestrator
    ):
    ...
    @pytest.mark.asyncio
    async def test_orchestrator_uses_cached_results_for_similar_query(
        self, orchestrator
    ):
    ...
    def test_requires_new_tools_identifies_tool_queries(self, orchestrator):
        ...
        assert (
            orchestrator._requires_new_tools("find publications by John Smith", context)
            == True
        )
        assert (
            orchestrator._requires_new_tools(
                "search for machine learning papers", context
            )
            == True
        )
        assert (
            orchestrator._requires_new_tools("show me research on AI", context) == True
        )
        assert (
            orchestrator._requires_new_tools(
                "get statistics about publications", context
            )
            == True
        )
        assert (
            orchestrator._requires_new_tools(
                "how many papers has John written", context
            )
            == True
        )
    ...
    def test_has_relevant_cached_results_finds_matches(self, orchestrator):
        ...
        assert (
            orchestrator._has_relevant_cached_results(
                "find publications by John Smith", context
            )
            == True
        )
        ...
        assert (
            orchestrator._has_relevant_cached_results(
                "search for machine learning", context
            )
            == False
        )
    ...
    async def test_orchestrator_handles_executor_failure(self, orchestrator):
        ...
        orchestrator.executor.execute_plan = AsyncMock(
            return_value=ExecutionResult(
                plan_id="test_plan",
                success=False,
                results={},
