...
class TestPublicationSearchInput:
    ...
    def test_invalid_year_range_future(self):
        ...
        with pytest.raises(
            ValueError, match=f"Year range must be between 1900 and {current_year + 1}"
        ):
...
class TestSearchPublicationsTool:
    ...
    def sample_es_response(self, realistic_publications):
        ...
        return {
            ...
            "hits": {
                ...
                "max_score": 1.5,
                "hits": [
                    {"_id": "pub-1", "_score": 1.5, "_source": es_formatted_pub},
                    {
                        "_id": "pub-2",
                        "_score": 1.2,
                        "_source": {
    ...
    async def test_execute_success(self, tool, mock_es_client, sample_es_response):
        ...
        result = await tool.execute(
            query="machine learning", year_range=(2020, 2023), limit=10
        )
    ...
    async def test_execute_validation_error(self, tool):
        ...
        result = await tool.execute(year_range=(2023, 2020))  # Invalid range
...
class TestSearchPublicationsLangChainTool:
    ...
    async def test_search_publications_impl_success(self):
        ...
        with patch(
            "src.tools.elasticsearch.publications.get_es_client"
        ) as mock_get_client:
            with patch(
                "src.tools.elasticsearch.publications.SearchPublicationsTool"
            ) as MockTool:
                # Setup mocks
                mock_client = Mock()
                ...
                result = await search_publications_impl(query="test", limit=5)
