...
class TestLLMFactory:
    ...
    def test_create_orchestrator_llm_success(self, mock_env_vars, mock_chat_litellm):
        ...
        with patch.dict(os.environ, mock_env_vars):
            with patch(
                "src.utils.llm_factory.ChatLiteLLM", return_value=mock_chat_litellm
            ):
    ...
    def test_create_orchestrator_llm_missing_api_key(self):
        ...
        with patch.dict(os.environ, {"LITELLM_BASE_URL": "https://test.api.com"}):
            ...
            assert "LITELLM_API_KEY environment variable is required" in str(
                exc_info.value
            )
    ...
    def test_create_orchestrator_llm_missing_base_url(self):
        ...
        with patch.dict(os.environ, {"LITELLM_API_KEY": "test-key"}):
            ...
            assert "LITELLM_BASE_URL environment variable is required" in str(
                exc_info.value
            )
...
class TestLiteLLMHelper:
    ...
    @pytest.mark.unit
    def test_validate_model_availability_success(
        self, mock_env_vars, mock_models_response
    ):
    ...
    @pytest.mark.unit
    def test_validate_model_availability_not_found(
        self, mock_env_vars, mock_models_response
    ):
    ...
    def test_validate_configuration_missing_base_url(self):
        ...
        with patch.dict(os.environ, {"LITELLM_API_KEY": "test-key"}):
            ...
            assert result["valid"] is False
            assert (
                "LITELLM_BASE_URL environment variable is missing" in result["errors"]
            )
            assert result["available_models"] is None
...
class TestLLMManager:
    ...
    @pytest.mark.unit
    def test_get_orchestrator_llm_creates_instance(
        self, mock_env_vars, mock_chat_litellm
    ):
    ...
    def test_manager_handles_llm_creation_errors(self, mock_env_vars):
        ...
        with patch.dict(os.environ, mock_env_vars):
            with patch(
                "src.utils.llm_factory.ChatLiteLLM",
