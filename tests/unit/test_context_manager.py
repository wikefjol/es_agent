...
class TestContextManager:
    ...
    def test_store_result_generates_unique_cache_keys(
        self, context_manager, sample_result
    ):
    ...
    def test_find_relevant_results_returns_similar_results(
        self, mock_similarity, context_manager, sample_cached_result
    ):
    ...
    @patch("src.core.context_manager.ContextManager._calculate_similarity")
    def test_find_relevant_results_filters_by_threshold(
        self, mock_similarity, context_manager, sample_cached_result
    ):
    ...
    def test_find_relevant_results_filters_by_session(
        self, context_manager, sample_cached_result
    ):
    ...
    def test_get_conversation_summary_returns_summary_for_session(
    ...
    def test_get_conversation_summary_returns_empty_for_nonexistent_session(
        self, context_manager
    ):
    ...
    def test_generate_cache_key_creates_consistent_keys(self, context_manager):
        ...
        key1 = context_manager._generate_cache_key(
            session_id="test", tool_name="search", parameters={"query": "test"}
        )
    ...
    def test_generate_cache_key_creates_different_keys_for_different_inputs(
        self, context_manager
    ):
        """Test that cache key generation creates different keys for different inputs."""
        key1 = context_manager._generate_cache_key(
            session_id="test1", tool_name="search", parameters={"query": "test"}
        )

        key2 = context_manager._generate_cache_key(
            session_id="test2", tool_name="search", parameters={"query": "test"}
        )

        assert key1 != key2

    def test_is_cache_entry_expired_correctly_identifies_expired_entries(
        self, context_manager
    ):
