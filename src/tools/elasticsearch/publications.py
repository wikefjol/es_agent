...
class PublicationSearchInput(BaseModel):
    ...
    query: Optional[str] = Field(
        None, description="Free text search in title and abstract fields"
    )
    author_name: Optional[str] = Field(
        None, description="Author name to search for (searches in nested Persons)"
    )
    year_range: Optional[Tuple[int, int]] = Field(
        None, description="Year range as tuple (start_year, end_year)"
    )
    ...
    def validate_year_range(cls, v):
        ...
        if v:
            ...
            if start < 1900 or end > current_year + 1:
                raise ValueError(
                    f"Year range must be between 1900 and {current_year + 1}"
                )
            if start > end:
...
class SearchPublicationsTool(ElasticsearchBaseTool):
    ...
    def _build_query(self, params: PublicationSearchInput) -> Dict[str, Any]:
        ...
        if params.publication_type:
            must_clauses.append(
                {"term": {"PublicationType.NameEng": params.publication_type}}
            )

        # Keywords filter
        if params.keywords:
            must_clauses.append({"terms": {"Keywords.Value": params.keywords}})

        # Organization filter
        if params.organization_id:
            must_clauses.append(
                {"term": {"AffiliatedIdsChalmers": params.organization_id}}
            )
    ...
    def _format_publication(self, hit: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "keywords": keywords,
            "doi": (
                source.get("IdentifierDoi", [None])[0]
                if source.get("IdentifierDoi")
                else None
            ),
            "scopus_id": (
                source.get("IdentifierScopusId", [None])[0]
                if source.get("IdentifierScopusId")
                else None
            ),
    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            response = await self._execute_search(
                index="research-publications-static", body=query_body, size=params.limit
            )
            ...
            return ToolResult(
                success=True,
                data={"publications": publications, "total_results": total_hits},
                metadata=metadata,
            )

        except ValueError as e:
            return ToolResult(
                success=False, data=None, error=f"Invalid input: {str(e)}"
            )
        except ElasticsearchError as e:
            return ToolResult(
                success=False, data=None, error=f"Elasticsearch error: {str(e)}"
            )
